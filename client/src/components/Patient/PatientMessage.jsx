...
import Sidebar from "./shared/patientSidebar";
...
export default function PatientMessage() {
  ...
  const [chatId, setChatId] = useState(user._id + id);
  const [currentMessage, setCurrentMessage] = useState("");
  const [messageList, setMessageList] = useState([]);

  const [doctor, setDoctor] = useState(); // contains detail of doctor by id

  const getDoctorDetails = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_SERVER_DOMAIN}/viewdoctorprofile/${id}`
      );
  ...
  const getAllChats = async () => {
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_SERVER_DOMAIN}/patient/getAllChats`,
        {
          chatId: chatId,
        },
        {
  ...
  const sendMessage = async () => {
    await socket.emit("get_all_past_data", chatId);
    socket.on("previouse_message", (data) => {
      setMessageList(data);
    });
    if (currentMessage !== "") {
      const messageData = {
